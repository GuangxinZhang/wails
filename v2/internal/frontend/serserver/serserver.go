...
func (d *DevWebServer) Run(ctx context.Context) error {
	...
	if devServerAddr := d.devServerAddr; devServerAddr != "" {
		...
		d.LogDebug("Serving Server at http://%s", devServerAddr)
	}
	// Launch desktop app
...
func (d *DevWebServer) LogDebug(message string, args ...interface{}) {
	d.logger.Debug("[WebServer] "+message, args...)
}
...
func NewFrontend(ctx context.Context, appoptions *options.App, myLogger *logger.Logger, appBindings *binding.Bindings, dispatcher frontend.Dispatcher, desktopFrontend frontend.Frontend) *DevWebServer {
	result := &DevWebServer{
		ctx:         ctx,
